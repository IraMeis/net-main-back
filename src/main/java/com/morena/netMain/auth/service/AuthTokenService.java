...
public class AuthTokenService {
    ...
    public JwtResponse login(JwtRequest authRequest) {
        if(authRequest == null)
            return buildSimpleAnswer("Невалидные данные");

        Optional<SysUsers> sysUser = authUserService.getSysUserByLogin(authRequest.getLogin());
        if(sysUser.isEmpty())
        ...
        if (user.getPassword().equals(authRequest.getPassword())) {
            ...
            return buildSimpleAnswer("Неправильный пароль");
        }
    }
